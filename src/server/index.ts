...
const TWELVEDATA_REST_URL = "https://api.twelvedata.com/time_series";
...
function isRestInterval(value) {
    return REST_INTERVALS.includes(value);
}
function historicalCandleKey(interval, datetime) {
    return `hist:${interval}:${datetime}`;
}
function historicalPrefix(interval) {
    return `hist:${interval}:`;
}
function historicalMetaKey(interval) {
    return `meta:${interval}`;
}
function normalizedOneHourKey(datetime) {
...
function parseCairoDatetimeParts(datetime) {
...
function cairoDatetimeToMs(datetime) {
...
function addMinutesToCairoDatetime(datetime, minutes) {
    const ms = cairoDatetimeToMs(datetime);
    if (ms === null)
        return datetime;
    return cairoTime(ms + minutes * 60_000);
}
function cairoDayOfWeek(datetime) {
    const parts = parseCairoDatetimeParts(datetime);
    if (!parts)
        return null;
...
function isClosedMarketCairoDatetime(datetime) {
    const parts = parseCairoDatetimeParts(datetime);
    ...
    if (day === 6 || day === 0)
        return true;
    if (day === 1 && parts.hour < 1)
        return true;
    // Daily closure: 00:00 -> 01:00 Cairo.
    return parts.hour < 1;
}
function statusFromExpectedClose(expectedCloseTime) {
...
export class Chat extends DurableObject {
    ...
    constructor(ctx, env) {
        ...
        ctx.blockConcurrencyWhile(async () => {
            const saved = await ctx.storage.get("live_state");
            if (saved) {
    ...
    async fetch(request) {
        ...
        if (url.pathname === "/price") {
            ...
            return json({
                ...
                tick_time: this.lastTickMs !== null
                    ? cairoTime(this.lastTickMs)
                    : null,
                age_seconds: this.lastTickMs !== null
        ...
        if (url.pathname === "/candles") {
            ...
            return json({
                ...
                confirmed: false,
                note: "Tick-built candles are provisional. REST historical candles are authoritative.",
                closed_candles: this.candles.slice(-limit),
                current_candle: this.currentCandle,
                last_price: this.lastPrice,
                last_tick_time: this.lastTickMs !== null
                    ? cairoTime(this.lastTickMs)
                    : null,
            });
        }
        if (url.pathname === "/sync") {
            const requestedInterval = url.searchParams.get("interval") ?? "1h";
            let outputsize = Number(url.searchParams.get("outputsize") ?? 100);
            ...
            if (requestedInterval === "all") {
                ...
                for (const interval of REST_INTERVALS) {
                    results.push(await this.syncHistoricalInterval(interval, outputsize, null));
                }
                return json({
                    status: "ok",
                    mode: "all",
                    symbol: SYMBOL,
                    timezone: TIMEZONE,
                    note: "Persistent raw REST storage only. No FVG, liquidity, sweep, mitigation, or market-structure analysis is performed in the Worker.",
                    results,
                });
            }
            if (!isRestInterval(requestedInterval)) {
                return json({
                    ...
                    note: "3min is intentionally not requested from Twelve Data. It will be derived later from valid 1min data.",
                }, 400);
            }
            const before = url.searchParams.get("before");
            const result = await this.syncHistoricalInterval(requestedInterval, outputsize, before);
        ...
        if (url.pathname === "/purge") {
            ...
            if (confirm !== "yes") {
                return json({
                    ...
                    example: `/purge?interval=${interval}&confirm=yes`,
                }, 400);
            }
            const deleted = await this.purgeHistoricalInterval(interval);
            return json({
                status: "ok",
                interval,
                deleted_raw_candles: deleted,
        ...
        if (url.pathname === "/data") {
            ...
            const candles = Array.from(rows.values()).sort((a, b) => b.datetime.localeCompare(a.datetime));
            const meta = (await this.ctx.storage.get(historicalMetaKey(interval))) ?? null;
            return json({
                status: "ok",
                symbol: SYMBOL,
                timezone: TIMEZONE,
        ...
        if (url.pathname === "/normalize") {
            ...
            if (interval !== "1h") {
                return json({
                    status: "error",
                    error: "Stage 1 normalization currently supports 1h only.",
                    note: "Other timeframes will use the same managed-data rules after the 1h session/gap engine is verified.",
                }, 400);
            }
        ...
        if (url.pathname === "/normalized-data") {
            ...
            if (interval !== "1h") {
                return json({
                    status: "error",
                    error: "Stage 1 normalized storage currently supports 1h only.",
                }, 400);
            }
            let limit = Number(url.searchParams.get("limit") ?? 100);
            if (!Number.isInteger(limit) || limit < 1) {
            ...
            const candles = Array.from(rows.values()).sort((a, b) => b.datetime.localeCompare(a.datetime));
            const meta = (await this.ctx.storage.get(normalizedOneHourMetaKey())) ?? null;
            return json({
                status: "ok",
                symbol: SYMBOL,
                timezone: TIMEZONE,
        ...
        if (url.pathname === "/storage-state") {
            ...
            for (const interval of REST_INTERVALS) {
                const meta = (await this.ctx.storage.get(historicalMetaKey(interval))) ?? null;
                const newest = await this.ctx.storage.list({
                    prefix: historicalPrefix(interval),
                    reverse: true,
                    limit: 1,
                });
                const oldest = await this.ctx.storage.list({
                    prefix: historicalPrefix(interval),
                    limit: 1,
                });
            ...
            return json({
                ...
                note: "3min storage is not created yet. Session filtering, synthetic gap candles, and derived timeframe logic will be added in the next stage.",
                frames,
            });
        }
        if (url.pathname === "/historical") {
            ...
            const before = url.searchParams.get("before");
            let outputsize = Number(url.searchParams.get("outputsize") ?? 10);
            if (!Number.isInteger(outputsize) || outputsize < 1) {
                outputsize = 10;
            }
            outputsize = Math.min(outputsize, 1150);
            const result = await this.fetchTwelveDataTimeSeries(interval, outputsize, before);
            return json(result, result.status === "ok" ? 200 : 502);
        }
        ...
        return json({
            ...
            endpoints: {
                ...
                historical: "/historical?interval=1h&outputsize=10",
                sync: "/sync?interval=1h&outputsize=100",
                sync_all: "/sync?interval=all&outputsize=100",
                data: "/data?interval=1h&limit=100",
                purge: "/purge?interval=1min&confirm=yes",
                normalize: "/normalize?interval=1h",
                normalized_data: "/normalized-data?interval=1h&limit=100",
            ...
            data_policy: {
                ...
                normalized_storage: "session-filtered candles + synthetic gap candles; no market-analysis logic",
                analysis: "ChatGPT only — Worker does not detect FVG, liquidity, sweeps, mitigation, or market structure",
            },
            historical_rest_source: "Twelve Data direct REST /time_series",
        });
    }
    ...
    async connect() {
        ...
        this.lastError = null;
        const endpoint = "wss://ws.twelvedata.com/v1/quotes/price" +
            "?apikey=" +
    ...
    async handleMessage(raw) {
        ...
        const tickMs = normalizeTimestamp(data.timestamp);
        this.processTick(price, tickMs);
    }
    processTick(price, tickMs) {
    ...
    createCandle(startMs, price, tickMs) {
        return {
            start_ms: startMs,
    ...
    async fetchTwelveDataTimeSeries(interval, outputsize, before) {
        try {
            if (!this.env.TWELVEDATA_API_KEY) {
                return {
                    status: "error",
            ...
            const upstream = new URL(TWELVEDATA_REST_URL);
            ...
            upstream.searchParams.set("apikey", this.env.TWELVEDATA_API_KEY);
            if (before) {
            ...
            const returnedInterval = String(payload.meta?.interval ?? "");
            if (returnedInterval &&
                returnedInterval !== interval) {
                return {
                    status: "error",
                    interval,
                    error: `Interval mismatch: requested ${interval}, Twelve Data returned ${returnedInterval}. Nothing was stored.`,
                };
            }
            ...
            const oldest = sorted.length > 0 ? sorted[0][0] : null;
            const newest = sorted.length > 0
                ? sorted[sorted.length - 1][0]
                : null;
            return {
                status: "ok",
                symbol: String(payload.meta?.symbol ?? SYMBOL),
                interval,
                count: candles.length,
    ...
    async syncHistoricalInterval(interval, outputsize, before) {
        try {
            const payload = await this.fetchTwelveDataTimeSeries(interval, outputsize, before);
            ...
            if (payload.interval !== interval) {
                return {
                    status: "error",
                    interval,
                    error: `Interval mismatch: requested ${interval}, received ${payload.interval}. Nothing was stored.`,
                };
            }
            const now = Date.now();
            const validRows = [];
            ...
            for (let i = 0; i < validRows.length; i += 100) {
                ...
                for (const candle of batch) {
                    entries[historicalCandleKey(interval, candle.datetime)] = candle;
                }
                await this.ctx.storage.put(entries);
            }
            const existingMeta = (await this.ctx.storage.get(historicalMetaKey(interval))) ?? null;
            const datetimes = validRows.map((c) => c.datetime).sort();
            const requestOldest = datetimes.length > 0 ? datetimes[0] : null;
            const requestNewest = datetimes.length > 0
            ...
            return {
                ...
                storage_layer: "raw_rest_persistent",
                historical_source: "Twelve Data direct REST",
                analysis_performed: false,
            };
        }
        catch (error) {
    ...
    async purgeHistoricalInterval(interval) {
        let deleted = 0;
        while (true) {
        ...
        await this.ctx.storage.delete(historicalMetaKey(interval));
        return deleted;
    }
    async normalizeStoredOneHour() {
        try {
            const raw = [];
            let startAfter;
            while (true) {
            ...
            for (const candle of raw) {
                const closedMarket = isClosedMarketCairoDatetime(candle.datetime);
                if (closedMarket) {
                    filteredClosedRows++;
                    if (lastValid && !gapOpen) {
                        gapOpen = {
                            start_datetime: addMinutesToCairoDatetime(lastValid.datetime, 60),
                ...
                if (gapOpen) {
                    ...
                    normalized.push({
                        ...
                        high: Math.max(gapOpen.previous_close, gapClose),
                        low: Math.min(gapOpen.previous_close, gapClose),
                        close: gapClose,
                        status: statusFromExpectedClose(gapExpectedClose),
                ...
                const expectedCloseTime = addMinutesToCairoDatetime(candle.datetime, 60);
                normalized.push({
                    timeframe: "1h",
                    ...
                    low: candle.low,
                    close: candle.close,
                    status: statusFromExpectedClose(expectedCloseTime),
                    source: "historical_rest",
                    provisional: false,
                    confirmed: true,
                    synthetic_gap: false,
            ...
            const oldNormalized = await this.ctx.storage.list({
                prefix: normalizedOneHourPrefix(),
                limit: 1000,
            });
            if (oldNormalized.size > 0) {
                await this.ctx.storage.delete(Array.from(oldNormalized.keys()));
            }
            for (let i = 0; i < normalized.length; i += 100) {
                const batch = normalized.slice(i, i + 100);
                const entries = {};
                for (const candle of batch) {
                    entries[normalizedOneHourKey(candle.datetime)] = candle;
                }
                await this.ctx.storage.put(entries);
            }
            ...
            const meta = {
                ...
                latest_datetime: datetimes.length > 0
                    ? datetimes[datetimes.length - 1]
                    : null,
                oldest_datetime: datetimes.length > 0
                    ? datetimes[0]
                    : null,
                raw_rows_seen: raw.length,
                valid_raw_rows: raw.length - filteredClosedRows,
                filtered_closed_rows: filteredClosedRows,
                synthetic_gap_rows: syntheticGapRows,
                pending_closed_period: pendingClosedPeriod,
                layer: "analysis_normalized",
            };
            await this.ctx.storage.put(normalizedOneHourMetaKey(), meta);
            return {
                status: "ok",
                symbol: SYMBOL,
                timezone: TIMEZONE,
                ...
                analysis_performed: false,
                note: "Worker only filters closed-market rows, inserts synthetic gap candles, and adds candle timing/status metadata. FVG, liquidity, sweeps, mitigation, and structure remain ChatGPT-only.",
            };
        }
        catch (error) {
            return {
    ...
    getState() {
        return {
            ...
            connection_status: this.connectionStatus,
            websocket_open: Boolean(this.ws &&
                this.ws.readyState === WebSocket.OPEN),
            last_price: this.lastPrice,
            last_tick_timestamp: this.lastTickMs,
            last_tick_time: this.lastTickMs !== null
                ? cairoTime(this.lastTickMs)
                : null,
            last_tick_age_seconds: this.lastTickMs !== null
                ? Math.floor((Date.now() - this.lastTickMs) / 1000)
                : null,
            ticks_received: this.tickCount,
            reconnect_count: this.reconnectCount,
            stored_closed_1m_candles: this.candles.length,
            current_1m_candle: this.currentCandle,
            recent_1m_candles: this.candles.slice(-5),
            subscribe_status: this.subscribeStatus,
            last_error: this.lastError,
            data_policy: {
                websocket: "PROVISIONAL",
                historical_rest: "AUTHORITATIVE",
                rule: "If REST historical data differs from tick-built candles, REST replaces the provisional data.",
            },
        };
    }
    async persist() {
...
export class Chat extends DurableObject<LiveEnv> {
	...
	async fetch(request: Request) {
		...
		if (url.pathname === "/purge") {
				...
				return json(
					{
						status: "error",
						error:
							"Deletion requires confirm=yes",
						example:
							`/purge?interval=${interval}&confirm=yes`,
					},
					400,
				);
			}
			...
			const deleted =
				await this.purgeHistoricalInterval(interval);
		...
		if (url.pathname === "/historical") {
			...
			const result = await this.fetchTwelveDataTimeSeries(
				interval,
				outputsize,
				before,
			);
		...
		return json({
			...
			historical_rest_source:
				"Twelve Data direct REST /time_series",
		});
	}
	...
	private async fetchTwelveDataTimeSeries(
		...
		try {
			...
			const payload =
				(await response.json()) as TwelveDataTimeSeriesPayload;
			...
			if (
				...
				return {
					...
					interval,
					error:
						`Interval mismatch: requested ${interval}, Twelve Data returned ${returnedInterval}. Nothing was stored.`,
				};
			}
			...
			const oldest =
				sorted.length > 0 ? sorted[0][0] : null;
	...
	private async syncHistoricalInterval(
		...
		try {
			const payload =
				await this.fetchTwelveDataTimeSeries(
					interval,
					outputsize,
					before,
				);
			...
			if (payload.interval !== interval) {
				return {
					...
					interval,
					error:
						`Interval mismatch: requested ${interval}, received ${payload.interval}. Nothing was stored.`,
				};
			}
